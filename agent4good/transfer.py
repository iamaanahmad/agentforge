...
def migrate_sqlite(source, backup, db):
    ...
    with sqlite3.connect(f"file:{source}?mode=ro", uri=True) as src, sqlite3.connect(backup) as dst:
        if src.execute("PRAGMA user_version").fetchone()[0] != 5:
            raise ValueError("Migration supports SQLite schema 5; upgrade the source offline first")
        src.backup(dst)
        if (
...
def backup_postgres(db, destination):
    with db.connect() as conn:
        ...
        document = {"version": 1, "tables": tables, "objects": objects}
...
def restore_postgres(db, source):
    ...
    if document.get("version") != 1 or set(document["tables"]) != set(TABLES + ["artifact_objects"]):
