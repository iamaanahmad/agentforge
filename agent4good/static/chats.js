...
async function refreshChat() {
  ...
  try {
    ...
    const html=data.messages.map(m=>`<article class="chat-message chat-${m.role}" data-message-id="${escapeHTML(m.id)}"><div class="chat-message-meta"><strong>${m.role==='user'?'You':m.role==='assistant'?escapeHTML(document.body.dataset.brandName || 'agentforge'):'Run update'}</strong><time>${time(m.created_at)}</time></div><div class="chat-message-body">${chatText(m.content)}</div>${m.needs_answer?'<p class="chat-question-label">Waiting for your answer below</p>':''}</article>`).join('');
